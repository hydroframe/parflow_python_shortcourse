...
##need writing tools and numpy
from parflow.tools.io import write_pfb, read_pfb
import numpy as np

wbx = Run("wbx_flux", __file__)
...
nt = int(wbx.TimingInfo.StopTime/wbx.TimeStep.Value)
...
# rain for 1 hour, recession for 1 hours

wbx.Cycle.rainrec.Names = 'r0 r1' #r2 r3 r4 r5 r6'
wbx.Cycle.rainrec.r0.Length = 5
wbx.Cycle.rainrec.r1.Length = 15
...
# because we are using a transient flux file we set the upper BC
# to be zero but we will use / convert the same flux below
wbx.Patch.z_upper.BCPressure.Type = 'OverlandFlow'
wbx.Patch.z_upper.BCPressure.Cycle = 'constant'
wbx.Patch.z_upper.BCPressure.alltime.Value = 0.0

#wbx.Patch.z_upper.BCPressure.r0.Value = rain_flux
...
#We set the transient flux forcing to true
wbx.Solver.EvapTransFileTransient = True
#we provide a base filename for these files, e.g. 'Forcing.00000.pfb' 
wbx.Solver.EvapTrans.FileName = 'Forcing' 

rainflux = np.zeros([30,30,30])
recflux = np.zeros([30,30,30])

# set upper cell of rainflux to rain value
# convert from  m / h of rainfall to flux 1/t units; not negative positive flip
# and divide by dz
rainflux[29,:,:]=(-rain_flux/wbx.ComputationalGrid.DZ)

# set upper cell of recflux to rain value
# convert from  m / h of rainfall to flux 1/t units; not negative positive flip
# and divide by dz
recflux[29,:,:]=(-rec_flux/wbx.ComputationalGrid.DZ)

## at this point we will save these NumPy arrays as distributed files
## so we need to get the path of the run directory and create it
...
wbx.Solver.Nonlinear.ResidualTol = 1e-8
...
wbx.Solver.Linear.Preconditioner = 'SMG'

#wbx.Solver.Linear.Preconditioner.PFMG.MaxIter = 10
